...
net = MyUNet(n_channels=1, n_classes=4)
...
SegModel = MySamFeatSeg(
    ...
    seg_decoder=MySegDecoderCNN(num_classes=4, num_depth=4)
)
...
for epoch in range(num_epochs):
    for data in tqdm(train):
        inputs, labels = data
        inputs = inputs.repeat(1, 3, 1, 1)  # This repeats the 1-channel data across 3 channels
        
        inputs, labels = inputs.to(device), labels.to(device)
        optimizer.zero_grad()
        inputs=inputs.reshape(BZ,3,256,256)
        outputs = model(inputs)
        pred_softmax = F.softmax(outputs, dim=1)
        loss = criterion(outputs, labels.long()) + dice_loss(pred_softmax, labels.long())
        loss.backward()
        optimizer.step()
    print(f'Epoch {epoch+1}/{num_epochs}, Loss: {loss.item()}')
torch.save(model.state_dict(), 'model_final_Unet_loss_more.pth')
